...
$books = pg_fetch_all(pg_query($connection, "SELECT  DISTINCT  client_book_1.client_usuario_mail, book.titulo, book.autor, book.preco, book.imagem
FROM client_book_1
LEFT JOIN book on client_book_1.book_id = book.id
where  client_book_1.client_usuario_mail  = '$user'"));
?>
...
<?php if ($books != null) {
    foreach ($books as $key => $book): ?>
        <div class="book_compra">
            ...
            <span> <?php echo $book['titulo']; ?> <br>
            <p class><?php echo $book['autor']; ?></p></span>
                <span> <br><?php echo $book['preco'] . "€"; ?></span>
            </div>
        </div>
    <?php endforeach;
}
